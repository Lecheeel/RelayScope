...
RelayScope 是一个本地 LLM API 审计工具，专门用来发现中转站伪装、假模型、降质模型、暗改缓存倍率、usage 异常和客户端兼容性问题。它的目标很直接：在你把中转站接入 Codex、Claude Code 或其他 Agent 工作流之前，先测清楚这个 API 到底值不值得信任。
...
RelayScope 的初衷就是解决这个痛点：不要只看中转站怎么宣传，而是用一组可复现的探针去验证模型身份、能力表现、缓存行为和计费信号。它帮你在花更多钱之前，先判断这个中转站有没有掺水。
...
- 模拟 Codex、Claude Code、OpenAI Chat 和 Anthropic Messages 客户端配置
- 检测 OpenAI 兼容与 Anthropic 原生协议行为
...
输入 API 地址、API Key、客户端配置和模型名称即可开始检测。API Key 只会发送到本机后端用于本次检测请求。
...
- 检测结果可能包含响应片段和中转站元数据，分享前请先检查报告内容。

## 点个 Star

如果 RelayScope 帮你避开了掺水中转站、假模型、降质模型或可疑计费，欢迎点个 Star。它能让更多开发者在付费和接入之前，先把 LLM API 测清楚。
...
Enter the API URL, API key, client configuration, and model. API keys are sent only to the local backend for the current probe request.
...
- Probe results may contain response excerpts and service metadata, so review reports before sharing them.
